server <- function(input, output, session) {
  ...
  output$jobDynamicText <- renderUI({
    ...
    HTML(paste0(
      ...
      " compared to November 2021 to January 2022.",
      "</p>",
      "Between ",
      format(jobTextData$date_popRate %m-% years(1), "%B %Y"),
      " and ",
  ...
  output$jobTimeComment <- renderUI({
    ...
    case_when(
      ...
      input$jobMetric == "growthRate" ~ paste0(
        "This chart shows the change in online job adverts since January 2022, presented as a 3-month rolling average. Compared to November 2021 to January 2022, there were ",
        scales::percent(round2(abs(jobTextData$value_growthRate), 2), trim = FALSE),
