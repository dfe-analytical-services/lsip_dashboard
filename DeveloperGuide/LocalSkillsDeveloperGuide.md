...
The pre production (where we test the dashboard before publishing) is
here (you may need to get access):
...
- *Annual population survey* is released four times a year: 1) Jan 2)
  Apr 3) Jul/Aug/Sept 4) Oct. We update employment volumes and split by
  industry at every release. However:

  - Highest qualification is only updated in the APS once a year in the
    Apr release.

  - We are not currently updating Employment by Occupation until the SOC
    issue is sorted.
...
- *Business demography* is released and updated in the dashboard once a
  year in Nov.

- *Destination measures* are released and updated in the dashboard once
  a year in Oct.
...
- 3-x are excel files which populate tables within the dashboard and a
  3-2_dataText helps populates the dynamic text in the Local Skills tab
...
some of the datasets currently have hardcoded dates because ONS are not
updating Occupation and Qualification data while they fix some problems.
Once those problems are fixed the code can be changed to use the LATEST
format used in the other Nomis datasets.
...
- Check that your dataset has been updated correctly (by comparing with
  the version you branched off of and cross checking with the raw data).
...
- You might need to make some adjustments to how it is presented in the
...
- You may want to add to the Overview page. If so, add to ui.R wherever
  ...
  function: createOverviewKPI, createOverviewChart, renderOverviewChart.
...
- updating boundaries (pages like this are good to keep an eye on
  ...
  but changes to LSIPs, MCAs and LEPs you are better off looking at the
