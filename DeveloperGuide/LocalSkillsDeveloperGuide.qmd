...
| Jan          | F   | M   | Apr          | M   | J   | Jun          | A   | Sep       | Oct          | Nov | D   |
|--------------|-----|-----|--------------|-----|-----|--------------|-----|-----------|--------------|-----|-----|
| APS emp vols |     |     | APS emp vols |     |     | APS emp vols |     | Bus count | APS emp vols | ILR |     |
| APS emp ind  |     |     | APS emp ind  |     |     | APS emp ind  |     |           | APS emp ind  |     |     |
|              |     |     | APS emp qual |     |     |              |     |           | Destinations |     |     |
...
2.  Download the latest version of the dataset. If you need help finding the dataset source, have a look at the "Data Sources" tab in the dashboard - this gives info and links to data sources.
...
4.  Navigate to /Data folder (either in R Files or in Windows Explorer). All raw datasets are held in /Data. They are the numbered datasets.
...
5.  Once you have figured out where the dataset you want to update is currently kept, navigate to that folder

6.  Delete the currently held dataset(s) in that folder. The folder should now be empty.

7.  Paste the new dataset in that folder.

8.  Run the ExtractLoadData.R script. This goes through all the numbered folders in /Data and loads the datasets in each folder as a R object.
...
10. Run the dashboard: `runApp()`

11. QA. Create a new QA log in "Analytical Projects\\Analysis\\S005 LSIP dashboard\\Documentation\\QA". Follow the structure of one already in there. Some checks you may do:
...
| *Headings*    | geogConcat              | metric               | breakdown                                                                                                              | subgroup                                                                     | chartPeriod                                            | timePeriod                    | latest                                                                                        | valueText                                                                         | value                                                                        |
|---------------|-------------------------|----------------------|------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------|--------------------------------------------------------|-------------------------------|-----------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------|------------------------------------------------------------------------------|
| *Description* | Area name and geography | Variable of interest | *If* the dataset has a breakdown they are listed here. Every metric **must** have a "Total" as well as any breakdowns. | Any subgroups of the breakdown. If breakdown is "Total" this is also "Total" | The period which the data relates to in plain English. | The start date of the period. | 1 if this is the most recent period in the data. -1 is one year before that. 0 for any other. | The value of the metric in this subgroup. Includes any supression from the source | The value of the metric in this subgroup in numeric terms. NA if suppressed. |
| *Format*      | character               | character            | character                                                                                                              | character                                                                    | character                                              | date                          | numeric                                                                                       | character                                                                         | numeric                                                                      |
| *Example*     | Black Country LEP       | inemployment         | Age                                                                                                                    | 19-24                                                                        | AY20/21                                                | 01/08/2020                    | -1                                                                                            | 5000                                                                              | 5000                                                                         |
