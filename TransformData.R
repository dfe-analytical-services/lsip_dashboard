...
formatNomis <- function(x) {
  x%>%
    ...
    fill(chartPeriod) %>% # fill time periods for all rows
    
    #sort out column names
    mutate(chartPeriod = ifelse(row_number() == which(grepl("Area", x[[1]]))[1], "chartPeriod",chartPeriod)) %>%#set to year so becomes column name
    ...
    #create geog columns
    mutate(geogConcat=case_when(geographicLevel=="COUNTRY" ~ area,
...
## 2.4 Enterprise by employment size ----
...
qualAgeGender <- formatNomis(I_qualAgeGender)%>%
  ...
  mutate(subgroup=case_when(subgroup=="" ~ "Total",
                            TRUE ~ trimws(str_to_sentence(subgroup),"both")),
         breakdown=case_when(subgroup %in% c("Males","Females") ~ "Gender",
...
C_qualL3PlusAgeGender<-qualAgeGender%>%
  ...
  left_join(qualSum)%>%
  mutate(metric="L3PlusPerc",value=qualL3Plus/allQuals)%>%
...
F_FeProvLevelAge<-I_FeProvLevelAge%>%
  ...
  filter(geographic_level %in% c("Local authority district","National"),#just keep area used
         (level_or_type=="Further education and skills: Total" | age_group=="Total"))%>% #keep only the combinations shown in dashboard
  mutate(area=case_when(geographic_level=="National" ~ country_name,
...
addGeogs <- function(x) {
  ...
  bind_rows(
    withAreas%>% 
      mutate(geogConcat=case_when(geographic_level=="National" ~ area,
...
groupedStats<-feWithAreas %>%
  ...
  mutate(starts_rate_per_100000_population = as.character(100000 * starts_1 / population_1),
...
C_FeProvLevelAge<-bind_rows(groupedStats,
                            feWithAreas %>%
                              filter(newArea==0))%>%
  select(-population,-newArea)%>%
  #get in new format
  mutate(subgroup=case_when(level_or_type=="Further education and skills: Total"&age_group=="Total" ~"Total",
  ...
  mutate(breakdown=case_when(level_or_type=="Further education and skills: Total"&age_group=="Total" ~"Total",
                             age_group!="Total" ~ "Age",
...
#add back on original LADUs and format
C_FeSsa<-bind_rows(groupedStats,
                   feSsaWithAreas %>%
                     filter(newArea==0))%>%
...
destinationsWithAreas<-
  #combine dataset
  bind_rows(I_KS4%>%mutate(metric="sustainedPositiveDestinationKS4Rate"),
...
#add back on original LADUs and format
C_destinations<-bind_rows(groupedStats,
                            destinationsWithAreas %>%
                              filter(newArea==0))%>%
  select(-newArea)%>%
  #get metrics
  mutate(Total=(education+appren+all_work)/cohort,
  ...
  mutate(breakdown=case_when((subgroup=="Total"&(is.na(cohort_level_group)==TRUE|cohort_level_group=="Total")) ~ "Total",
...
# format data
F_adverts <- bind_rows(
  formatVacancies(I_OnsProfDetailLep), # remove code since it doesn't exist in other geogs
  formatVacancies(I_OnsProfDetailLsip),
  formatVacancies(I_OnsProfDetailMca),
  formatVacancies(I_OnsProfDetailLA %>% rename(region = 1) %>% filter(!(region %in% c("Scotland", "Wales", "Northern Ireland", "Unknown"))) %>% select(-region)), # remove region code
  formatVacancies(I_OnsProfDetailEng) %>% filter(area == "England"),
  formatVacancies(I_OnsProfDetailRegion) %>% filter(area == "London") %>% mutate(geographic_level = "LSIP", area = "Greater London"),
  formatVacancies(I_OnsProfDetailRegion) %>% filter(area == "London") %>% mutate(geographic_level = "LEP"),
  ...
  formatVacancies(I_OnsProfLA %>% rename(region = 1) %>% filter(!(region %in% c("Scotland", "Wales", "Northern Ireland", "Unknown"))) %>% select(-region) %>% mutate(`Detailed Profession Category` = "Detailed profession category")) %>% mutate(`Detailed Profession Category` = "None"),
  ...
  formatVacancies(I_OnsProfRegion %>% mutate(`Detailed Profession Category` = "Detailed profession category")) %>% mutate(`Detailed Profession Category` = "None") %>% filter(area == "London") %>% mutate(geographic_level = "LSIP", area = "Greater London"),
  formatVacancies(I_OnsProfRegion %>% mutate(`Detailed Profession Category` = "Detailed profession category")) %>% mutate(`Detailed Profession Category` = "None") %>% filter(area == "London") %>% mutate(geographic_level = "LEP")
) %>%
...
formatBusiness <- function(x,y) {
  ...
  colnames(x)[2] <- "area" 
  x%>%pivot_longer(!c("areaCode","area"),names_to = "chartPeriod", values_to = "value")%>%
    filter(substr(areaCode,1,2)=="E0"|areaCode=="E92000001")%>%
    mutate(metric=y,geographic_level=case_when(areaCode=="E92000001"~"National",TRUE ~ "LADU")
           ,area=case_when(areaCode=="E92000001"~"England",TRUE ~ trimws(area)),
           areaCode=trimws(areaCode))
  }
...
#add back on original LADUs and format
C_businesses<-bind_rows(groupedStats,
                        businessesWithAreas %>%
                          filter(newArea==0))%>%
  select(-newArea)%>%
  #unpivot to get rates
  pivot_wider(names_from = metric, values_from = value)%>%
  mutate(birthRate=births/(births+deaths+active),
